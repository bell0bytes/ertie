...
class ZitadelIntrospectTokenValidator(IntrospectTokenValidator):
    def __init__(self, conf):
        super().__init__()
        self._zitadelDomain = conf['ZITADEL_DOMAIN']
        self._clientID = conf['ZITADEL_CLIENT_ID']
        self._clientSecret = conf['ZITADEL_CLIENT_SECRET']

    def introspect_token(self, token_string):
        url = f'{self._zitadelDomain}/oauth/v2/introspect'
        data = {'token': token_string, 'token_type_hint': 'access_token', 'scope': 'openid'}
        auth = HTTPBasicAuth(self._clientID, self._clientSecret)
